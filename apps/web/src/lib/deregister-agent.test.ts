...
describe("deregisterAgentForUser", () => {
  ...
  it("surfaces a clear message when the agent has a pending update (409)", async () => {
    createAdminPaymentNodeClientMock.mockReturnValue({
      deregisterAgent: vi
        .fn()
        .mockRejectedValue(
          new Error(
            "409: Agent registration cannot be deregistered in its current state: UpdateRequested",
          ),
        ),
    });
    resolveSmartContractAddressForDeregisterMock.mockResolvedValue(
  ...
  it("passes through other 409 state-conflict reasons", async () => {
    createAdminPaymentNodeClientMock.mockReturnValue({
      deregisterAgent: vi
        .fn()
        .mockRejectedValue(
          new Error(
            "409: Agent registration cannot be deregistered in its current state: DeregistrationRequested",
          ),
        ),
    });
    resolveSmartContractAddressForDeregisterMock.mockResolvedValue(
    ...
    expect(result).toEqual({
      success: false,
      error:
        "Agent registration cannot be deregistered in its current state: DeregistrationRequested",
    });
  });
