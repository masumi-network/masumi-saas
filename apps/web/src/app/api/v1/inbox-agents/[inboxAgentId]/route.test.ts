...
describe("DELETE /pay/api/v1/inbox-agents/:id", () => {
  ...
  it("deletes through the admin payment-node key after ownership and state checks pass", async () => {
  ...
  it("does not create the admin client when ownership is missing", async () => {
  ...
  it("returns 503 when the admin payment-node client is not configured", async () => {
    createInboxAdminPaymentNodeClientMock.mockImplementation(() => {
      throw new PaymentNodeConfigError(
        "PAYMENT_NODE_ADMIN_API_KEY is required for admin operations",
      );
    });
    getOwnedInboxAgentForUserMock.mockResolvedValue({
      source: "db",
    ...
    await expect(response.json()).resolves.toStrictEqual({
      success: false,
      error: "PAYMENT_NODE_ADMIN_API_KEY is required for admin operations",
    });
    expect(deleteInboxAgentReferenceMock).not.toHaveBeenCalled();
  });
});
