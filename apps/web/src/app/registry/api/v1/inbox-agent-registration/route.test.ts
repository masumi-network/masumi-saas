...
describe("/registry/api/v1/inbox-agent-registration", () => {
  ...
  it("forwards inbox registration browse requests", async () => {
    ...
    const request = new NextRequest(
      "https://saas.example.com/registry/api/v1/inbox-agent-registration?network=Preprod",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://registry.example.com/api/v1/inbox-agent-registration/?network=Preprod",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ limit: 5 }),
      }),
